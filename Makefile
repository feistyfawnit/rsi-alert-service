.PHONY: up down logs test test-anomaly ps clean pnl-report remote-report remote-csv deploy remote-logs ship
...
remote-report:
	@mkdir -p reports
	@curl -s http://$(EC2_IP):8080/api/positions/pnl-report > reports/pnl-report.md
	@echo "P&L report pulled from EC2 → reports/pnl-report.md"

remote-csv:
	@mkdir -p reports
	@curl -s http://$(EC2_IP):8080/api/positions/pnl-report/csv > reports/signal-outcomes-live.csv
